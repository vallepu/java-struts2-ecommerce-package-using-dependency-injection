...
<script type='text/javascript' src='./includes/verticalnavigation.js'></script>
	...
	<script type="text/javascript">
		...
		function formatProductShippingLink(cellvalue, options, rowObject) 
		{
			
			 return "<a href='#' onClick='javascript:openShippingDialog("+cellvalue+")'>Shipping Details</a>";	
        }
		...
		function openShippingDialog(productid) 
		{
			$("#product_shipping_details").load("<%= request.getContextPath() %>/productshipping.action?id=" + productid);
			$("#product_shipping_details").dialog('open');
        }
...
<body>
  ...
  <s:url id="remoteurl" action="jsoncategoryaction"/> 
    ...
    >
    	...
    	<sjg:gridColumn name="id" index="id" hidden="false" title="Product ID" formatter="integer" sortable="false"/>
        ...
        <sjg:gridColumn name="name" searchoptions="{sopt:['eq']}" editable="true" index="name" title="Name" sortable="true"/>
         ...
         <sjg:gridColumn name="description" searchoptions="{sopt:['eq']}" editable="true" index="description" title="Description" sortable="true"/>
          <sjg:gridColumn name="price" searchoptions="{sopt:['eq']}" editable="true" index="price" title="Price" sortable="true"/>
      	 ...
      	 <sjg:gridColumn name="id" index="id" title="Shipping Details" edittype="select" align="center" hidden="false" formatter="formatProductShippingLink" key="true" sortable="false"/>
      	<sjg:gridColumn 
    		name="categoryid"
    		id="categoryid"
    		index="id"
